...
//     }
    ...
    var finish_score = localStorage.getItem("fin_score"); //抓名為fin_score的localStoragea裡的資料
    console.log(finish_score);
    var result02 = document.getElementById("score");
    let finish_score1 = parseInt(finish_score, 10);
    result02.innerHTML = finish_score1;
    
    if(finish_score1>=0 && finish_score1<=12){
        result02.classList.add('green');
    }else if (finish_score1>=13 && finish_score1<=22){
        result02.classList.add('yellow');
    }else if(finish_score1>=23 && finish_score1<=32){
        result02.classList.add('orange');
    }else if(finish_score1>=33){
        result02.classList.add('red');
    }
    ...
    if((count1 > count2) & (count1 > count3) & (count1 > count4) & (count1 > count5) & (count1 > count6)){
        ...
        result00.innerHTML = "<div class='result001'>”無症狀，日常可以飲用以下中藥食療保養。”</div>";
        var m0001 = document.getElementById("m0001");  //枸杞菊花茶
        m0001.addEventListener("click", m00011);
        function m00011(){
            document.getElementById("medicine").src = "../acu-front/medicine/00-01.html";
        }
    
        var m0002 = document.getElementById("m0002");  //枸杞夏枯草
        m0002.addEventListener("click", m00022);
        function m00022(){
            document.getElementById("medicine").src = "../acu-front/medicine/00-02.html";
        }
    
        var m0003 = document.getElementById("m0003");  //黃耆枸杞紅棗茶
        m0003.addEventListener("click", m00033);
        function m00033(){
            document.getElementById("medicine").src = "../acu-front/medicine/00-03.html";
        }
    
        if(document.getElementById("m0001").checked){
            document.getElementById("medicine").src = "../acu-front/medicine/00-01.html";
        }
    }else{
        medicine_button.innerHTML="<label><input id='m0001' class='mouse button' type='radio' name='MM' value='枸杞菊花茶' checked/><span class='medicine-button mouse'>枸杞菊花茶</span></label><label><input id='m0002' class='mouse button' type='radio' name='MM' value='枸杞夏枯草' /><span class='medicine-button mouse'>枸杞夏枯草</span></label><label><input id='m0003' class='mouse button' type='radio' name='MM' value='黃耆枸杞紅棗茶' /><span class='medicine-button mouse'>黃耆枸杞紅棗茶</span></label>"
        result04.innerHTML = "”與多症狀類型相符”";
         result00.innerHTML = "<div class='result001'>”⚠️與多症狀類型相符，日常可以多飲用以下中藥食療，進階中醫食療請向中醫師諮詢。⚠️”</div>";
